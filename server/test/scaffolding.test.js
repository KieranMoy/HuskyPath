...
test('POST /api/schedules stub returns 501 with contract', async () => {
  const server = await start();
  try {
    const port = server.address().port;
    const res = await fetch(`http://localhost:${port}/api/schedules`, {
      method: 'POST',
      headers: { 'content-type': 'application/json' },
      body: JSON.stringify({ courses: [], constraints: {}, topN: 3 }),
    });
    assert.equal(res.status, 501);
    const body = await res.json();
    assert.equal(body.endpoint, 'POST /api/schedules');
    assert.equal(body.owner, 'Kieran');
  } finally {
    server.close();
  }
});
